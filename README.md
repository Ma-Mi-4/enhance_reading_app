...
## 技術的な課題と対策
- AI問題生成の品質担保 → 手動で10問作成 → レビュー → 段階的にAI生成問題を追加
　手動作成問題品質基準：
　TOEIC Part 7の文書形式（メール・広告・記事等）に準拠
　600-800点レベルの語彙・文法
　250-350語の文章量
　4-6問の設問バランス
- 復習システム → 間違えた問題と解答に時間のかかった問題（1問あたり5分以上だったもの）から出題
- 通知機能の実装 → ユーザー設定の時間に通知 → ユーザーがよく使う時間帯に通知 → 通知にモチベーションを上げるメッセージも追加